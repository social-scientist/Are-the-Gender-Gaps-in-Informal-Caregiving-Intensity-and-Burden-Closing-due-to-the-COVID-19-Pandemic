...
*
*
*Are the gender gaps in informal caregiving intensity and burden 
*closing due to the COVID‐19 pandemic? Evidence from the Netherlands
*
...
use "\\CNAS.RU.NL\U679219\Documents\own data\si20a_EN_1.0p.dta", clear // Data from LISS from March 2020: retrieved from https://www.dataarchive.lissdata.nl/study_units/view/1067
numlabel, add
...
*******************************************************************************
*
*Part 3: Actual analysis for the paper
*
*******************************************************************************
clear
set more off
...
tab cov_2 // 2 means the people that stopped but not becasue of coronavirus
tab cov_4
******************************************************************************
*Sample restriction
table cov_1 cov_5
...
*dependent variables
******************************************************************************
*Change in caring intensity (care_int)
...
recode intensity (0 1 2 = 1 "less") (3 = 2 "same") ( 4 5 = 3 "more"), gen(intensity_cat)
tab intensity, m

******************************************************************************
*difference meassure: positive, negative, or no change
*variable in march: var17_f
*variable in july: cov_11a
*check
tab var17_f, m
*recode "weet niet" into missing
recode var17_f (6=.),gen(zwaar_march)
label variable zwaar_march "Burden in March"
tab zwaar_march, m

*check
tab cov_11a, m
*recode "weet niet" into missing
recode cov_11a (6=.),gen(zwaar_july)
label variable zwaar_july "Burden in April/May"
tab zwaar_july

*calculate difference
gen difference = zwaar_july - zwaar_march
tab difference, m

*recode to variable on burden change
recode difference (-4 -3 -2 -1 = 1 "less burden") (0 = 2 "same") (4 3 2 1 = 3 "more burden"), gen(burden_diff)
tab burden_diff, m
...
tab cov_12i
recode cov_12i (6=.), gen(burden_sub)

label define burden_sub 1 "much less" 2 "less" ///
3 "same" 4 "more" ///
5 "much more"
label values burden_sub burden_sub
...
tab geslacht, m
*recode into dummy 0/1
recode geslacht (1=0 "Men") (2=1 "Women"), gen(sex)
replace sex = 0 if nomem_encr == 873587 | nomem_encr ==  859707 // in the orignal data that we first received nomem_encr 873587 & 859707 were identifable as man which is why I here recode it manualy
label variable sex ""
tab sex, m // 859707
...
*age
tab leeftijd, m
replace leeftijd = 28 if nomem_encr == 873587 // in the orignal data that we first received nomem_encr 873587 was identifable as age 28 which is why I here recode it manualy
replace leeftijd = 71 if nomem_encr == 859707 // in the orignal data that we first received nomem_encr 873587 was identifable as age 28 which is why I here recode it manualy

*intensity march
tab var9_end, m //225 missings

*how many people cared for
bysort nomem_encr: gen pycount = _N
tab pycount

bysort nomem_encr: gen pynr = _n
tab pynr

xtset nomem_encr pynr


*how many women and men in the data set
tab sex if pynr == 1


***************************************************************************
save "gender_covid_after_data_prep_2.dta", replace

****************************************************************************
*March data
***************************************************************************
*Did women have more burden in March?
tab zwaar_march sex  

*march burden 
use "gender_covid_after_data_prep_2.dta", clear
keep if (sex != . & leeftijd != . & zwaar_march != .)

ologit zwaar_march sex leeftijd hulp_aantal, vce(cl nomem_encr)
...
margins, at(sex=(0/1)) predict(outcome(4)) atmeans
margins, at(sex=(0/1)) predict(outcome(5)) atmeans

*************
*Did women care more in March?

use "Mantelzorg_long_life_course.dta", clear
keep if var2==1
ttest var9_end, by(geslacht) //women care one hour on average more
...
*load data
use "gender_covid_after_data_prep_2.dta", clear
*listwise deletion on variables in model
keep if (sex != . & leeftijd != . & intensity != .)

*descriptive
tab intensity sex, col

*graph intensity with sex
gen helpi = 1
label define helpi 1 "Change in care intensity"
label values helpi helpi

*Figure 1
catplot sex intensity, percent(sex) over(helpi) bar(1, fcolor(gs0) lstyle(none)) bar(2, fcolor(gs8) lstyle(none)) asyvars recast(bar) ///
 graphregion(color(white)) /// 
 blabel(bar, format(%9.0f)) ///
ytitle("% of caregiving spells")
graph export cov_5_sex.png, replace

*************************************
*mulitnomial
tab sex, m
mlogit intensity sex leeftijd pycount, vce(cl nomem_encr) 
outreg2 using ${data}gender_COVID_multinom, word dec(3) alpha (0.001, 0.01, 0.05) ctitle(Model 1) label replace

********************************************************************
*subjective burden 
********************************************************************
*Analysis- higher number more burden

use "gender_covid_after_data_prep_2.dta", clear
*listwise deletion on variables in model
keep if (sex != . & leeftijd != .  & burden_sub != . & cov_1 == 1 )

gen helpi2 = 1
label define helpi2 1 "Subjective change in burden"
label values helpi2 helpi2

*descriptive
tab burden_sub

*Figure 2
catplot sex burden_sub, percent(sex) over(helpi2) bar(1, fcolor(gs0) lstyle(none)) bar(2, fcolor(gs8) lstyle(none)) asyvars recast(bar) ///
ytitle("% of caregiving spells") ///
 graphregion(color(white)) /// 
blabel(bar, format(%9.0f)) 
graph export burden_sub_sex.png, replace


ologit burden_sub sex leeftijd pycount, vce(cl nomem_encr)
estimates store Model2
outreg2 using ${data}gender_COVID, word dec(3) alpha (0.001, 0.01, 0.05) ctitle(Model 2) label 

*ologit burden_sub sex leeftijd pycount zwaar_march, vce(cl nomem_encr)

*************************************
*mulitnomial
tab sex
mlogit burden_sub sex leeftijd pycount, vce(cl nomem_encr)
outreg2 using ${data}gender_COVID_multinom_subjective, word dec(3) alpha (0.001, 0.01, 0.05) ctitle(Model 2) label 
...
use "gender_covid_after_data_prep_2.dta", clear
*listwise deletion on variables in model
keep if (sex != . & leeftijd != . & burden_diff != . & cov_1 == 1 & zwaar_july !=. & zwaar_march != .)

gen helpi3 = 1
label define helpi3 1 "Objective change in burden"
label values helpi3 helpi3

*descriptive
tab burden_diff

*crosstable
bysort sex: tab zwaar_march zwaar_july, col 
asdoc bysort sex: tab zwaar_july zwaar_march, col font(Times New Roman) replace

*Figure 3
catplot sex burden_diff, percent(sex) over(helpi3) bar(1, fcolor(gs0) lstyle(none)) bar(2, fcolor(gs8) lstyle(none)) asyvars recast(bar) ///
ytitle("% of caregiving spells") ///
 graphregion(color(white)) /// 
blabel(bar, format(%9.0f)) 
graph export burden_diff_sex.png, replace


ologit burden_diff sex leeftijd pycount, vce(cl nomem_encr)
estimates store Model3
margins, at(sex=(0/1)) predict(outcome(1)) atmeans
margins, at(sex=(0/1)) predict(outcome(2)) atmeans
margins, at(sex=(0/1)) predict(outcome(3)) atmeans
outreg2 using ${data}gender_COVID, word dec(3) alpha (0.001, 0.01, 0.05) ctitle(Model 3) label 

mlogit burden_diff sex leeftijd pycount, vce(cl nomem_encr)
outreg2 using ${data}gender_COVID_multinom_objective, word dec(3) alpha (0.001, 0.01, 0.05) ctitle(Model 2) label 
